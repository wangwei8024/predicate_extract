...
def save_checkpoint(models, optimizers, infos, is_best, opt):
    "save models, optimizer, and infos"
    ...
    def save(prefix):
        ...
        torch.save(model_params, prefix + 'models.pth')
    ...
    logger.info("Saving checkpoint to {}".format(prefix + 'models.pth'))
    save(prefix)
    # save best
    if is_best:
...
# load
def load_checkpoint(models, optimizers=None, opt=None):
    logger = logging.getLogger('__main__')
    # check compatibility if training is continued from previously saved models
    if opt.resume_from is None or os.path.isdir(opt.resume_from):
        logger.warning("resume_from not set, training from scratch")
        return False
    prefix = 'best_' if opt.resume_from_best else ''
    flag = True
    abspath = os.path.join(opt.checkpoint_root, opt.resume_from)
    # load models
    path = os.path.join(abspath, prefix + 'models.pth')
    print('path--models', path)
    if os.path.isfile(path):
        ...
        logger.warning("Fail to load models")
        flag = False

    # load optimizer
    if optimizers is not None:
        path = os.path.join(abspath, prefix + 'optimizer.pth')
        print('path--opt', path)
        if os.path.isfile(path):
...
def load_info(opt):
    ...
    abspath = os.path.join(opt.checkpoint_root, opt.resume_from)
    prefix = 'best_' if opt.resume_from_best else ''
    path = os.path.join(abspath, prefix + 'infos.pkl')
    if os.path.isfile(path):
        ...
        #         checkme], "Command line argument and saved models disagree on '%s' " % checkme
    else:
